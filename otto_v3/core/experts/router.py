...
from otto_v3.core.experts.protector import CONFIG as PROTECTOR
from otto_v3.core.experts.decomposer import CONFIG as DECOMPOSER
from otto_v3.core.experts.restorer import CONFIG as RESTORER
from otto_v3.core.experts.redirector import CONFIG as REDIRECTOR
from otto_v3.core.experts.acknowledger import CONFIG as ACKNOWLEDGER
from otto_v3.core.experts.guide import CONFIG as GUIDE
from otto_v3.core.experts.executor import CONFIG as EXECUTOR

# Registry is a tuple sorted by expert name at module load.
...
# Tuple of tuples for fixed order.
...
class NEXUSRouter:
    ...
    Args:
        ...
        learned_weights: Optional per-expert calibration weights from
            the BCM learning system (CalibrationLearner.get_weights()).
            Each value multiplies the expert's Phase 2 score before
            safety floors are applied in Phase 3. Defaults to 1.0
            (no effect) for any expert not in the dict.
    """
    ...
    def __init__(
        ...
        # Ensure experts are sorted by name
        self._experts = tuple(
            sorted(experts or ALL_EXPERTS, key=lambda e: e.name)
        )
        self._on_route = on_route
        self._learned_weights: dict[str, float] = learned_weights or {}
    ...
    def _weight(
        ...
        for expert in self._experts:  # Sorted by name
            ...
            # Apply BCM learned calibration weight (default 1.0 = no change)
            total *= self._learned_weights.get(expert.name, 1.0)
