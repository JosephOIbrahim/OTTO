...
ServiceRegistry iterates services in sorted name order.
...
class OTTOService(Protocol):
    ...
    def get_signals(self) -> list[CategoricalSignal]:
        ...
        Signals returned in sorted order by
        ``(category, value)``.
        """
        ...
...
class ServiceRegistry:
    ...
    Services are iterated in sorted name order.
    Signals returned in sorted ``(source, category, value)`` order.
    """
    ...
    def get_all_signals(self) -> list[CategoricalSignal]:
        ...
        Services queried in sorted name order.
